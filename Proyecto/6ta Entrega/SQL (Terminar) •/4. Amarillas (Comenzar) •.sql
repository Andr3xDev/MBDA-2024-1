...
-- Paquetes de plataformas

CREATE OR REPLACE PACKAGE PC_PLATAFORMAS AS
...
CREATE OR REPLACE PACKAGE BODY PC_USUARIOS AS
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20011, 'La actualización del usuario no se pudo completar debido a un error inesperado.');
    ...
    BEGIN
        ...
        ELSE
            DBMS_OUTPUT.PUT_LINE('No se encontró un comprador con ID ' || p_id);
        END IF;
...
CREATE OR REPLACE PACKAGE BODY PC_GENEROS AS
    ...
    BEGIN
        ...
        -- Obtener la información de los géneros
        LOOP
        FETCH c_genero INTO v_genero;
        EXIT WHEN c_genero%NOTFOUND;
        
        DBMS_OUTPUT.PUT_LINE('Género: ' || v_genero.genero);
        END LOOP;
...
-- Paquete de Plataformas
CREATE OR REPLACE PACKAGE BODY PC_PLATAFORMAS AS
...
CREATE OR REPLACE PACKAGE BODY PC_PRODUCTOS AS
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE('Condición: ' || v_producto.condicion);
...
CREATE OR REPLACE PACKAGE BODY PC_WISHLIST AS
    ...
    BEGIN
        ...
        EXIT WHEN filtered_cursor%NOTFOUND;  -- Salir del loop cuando no haya más filas
        
        -- Procesar cada fila (en este caso, simplemente imprimir los valores)
        DBMS_OUTPUT.PUT_LINE('Wishlist: ' || v_wishlist || ', Producto: ' || v_producto || 
                            ', Prioridad: ' || v_prioridad || ', Cantidad: ' || v_cantidad);
        END LOOP;
...
CREATE OR REPLACE PACKAGE BODY PC_INTERCAMBIOS AS
    ...
    BEGIN
        ...
        EXIT WHEN compradores_cursor%NOTFOUND;  -- Salir del loop cuando no haya más filas
        END LOOP;
...
CREATE OR REPLACE PACKAGE BODY PC_CALIFICACIONES AS
    ...
    BEGIN
        ...
        -- Actualizar el comentario de la calificación
        UPDATE calificaciones 
        SET comentario = c_comentario
        WHERE id_producto = productos.id AND id_comprador = comprador.id;
...
CREATE OR REPLACE PACKAGE BODY PC_PAGOS AS
    ...
    BEGIN
        ...
        EXIT WHEN pagos_cursor%NOTFOUND;  -- Salir del loop cuando no haya más filas
        
        -- Procesar cada fila (en este caso, simplemente imprimir los valores)
        DBMS_OUTPUT.PUT_LINE('');
        DBMS_OUTPUT.PUT_LINE('ID: ' || v_id);
        DBMS_OUTPUT.PUT_LINE('Método: ' || v_metodo);
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20105, 'No se encontró el pago especificado.');
        WHEN OTHERS THEN
